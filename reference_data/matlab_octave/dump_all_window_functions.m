...
for M = minM:maxM
    ...
    if ismember(source, {'not-in-matlab', 'octave'})
        % The gaussian() function has an optional parameter 'a' that defaults to 1.0 if not specified.
        % It is only available in Octave, not in Matlab.
        dump_window_function(fo, source, 'gaussian'                  , @(M) gaussian        (M             ), M);
        dump_window_function(fo, source, 'gaussian_1p0'              , @(M) gaussian        (M,  1.0       ), M);
        dump_window_function(fo, source, 'gaussian_2p5'              , @(M) gaussian        (M,  2.5       ), M);
    endif

    if ismember(source, {'matlab', 'octave'})
        % The gausswin() function has an optional parameter 'a' that defaults to 2.5 if not specified.
